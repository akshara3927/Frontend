...
def show_module_detail():
    code, module_name, module_desc, tables, records = st.session_state.selected_module
    cat_key = st.session_state.selected_category
    
    # Breadcrumb
    st.markdown(f"Category {cat_key.split('-')[0].strip()} > {module_name}")
    ...
    if code == "G4":
        ...
        tab = st.radio(
            "Navigation",
            ["Home", "My Records", "Consent Management", "Research Studies", "Privacy Policy"],
            horizontal=True
        )
        st.divider()
        ...
        if tab == "Home":
            ...
            st.info("Medical records view (existing)")

        elif tab == "Consent Management":
            consent_management_view()

        elif tab == "Research Studies":
            research_studies_view()
    ...
    # 🔽 DEFAULT (non-G4 users)
        ...
        if tab == "🏠 Home":
            st.info(f"**{module_name}** - {module_desc}")
